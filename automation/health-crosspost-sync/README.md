...
> `gov-thread-autopost`와 같은 이유로 별도 저장소가 아니라 이 저장소(`hidden-benefit`) 안
> `automation/health-crosspost-sync/` 폴더에 들어 있습니다. 기존 건강블로그 쓰레드 자동
> 발행(Make.com 시나리오)이나 gov-thread-autopost는 전혀 건드리지 않습니다 — 이 폴더는
> 완전히 독립적으로 동작하고, 결과물(`latest.json`)만 다른 시스템이 읽어가는 구조입니다.
...
`latest.json`을 조회하는 단계를 추가해서, 그 글의 제목이 방금 RSS로 읽은 최신 글과 같고
`blogspot_url`이 있으면 티스토리/블로그스팟 링크 중 하나를 무작위로 고르도록 했습니다.
...
GitHub 연결 권한 문제로 `.github/workflows/health-crosspost-sync.yml` 파일은 자동으로
못 올릴 수 있습니다. 그 경우 GitHub 웹에서 저장소 루트에 **Add file → Create new file**로
경로를 `.github/workflows/health-crosspost-sync.yml`로 입력하고, 이 폴더의
`workflow.yml.txt` 내용을 붙여넣어 커밋하세요.

### 2) 시크릿 등록 (인스타그램 자동 게시를 켜려면)
